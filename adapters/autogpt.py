...
class AutoGPTAdapter:
    ...
    def _build_event(self, event_type: EventType, **kwargs: Any) -> AgentEvent:
        return AgentEvent(
    ...
    async def emit_action(self, command: str, arguments: dict[str, Any] | None = None) -> AgentEvent:
    ...
    async def emit_result(self, output: Any, error: str | None = None) -> AgentEvent:
        ...
        event = self._build_event(
            event_type,
            status=status,
            tool_result=ToolResultData(tool_name=self.TOOL_NAME, output=output, error=error),
        )
        return await self._publish(event)
